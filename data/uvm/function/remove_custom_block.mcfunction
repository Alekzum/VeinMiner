...
$execute if score removed uvm.temp matches 1 run tellraw @a ["", {"text": "[UVM] ", "color": "gold"}, {"text": "Removed $(namespace):$(id) from vein mining blocks!", "color": "green"}]
$execute if score removed uvm.temp matches 0 run tellraw @a ["", {"text": "[UVM] ", "color": "gold"}, {"text": "Block $(namespace):$(id) not found in custom blocks list.", "color": "red"}]
